    ...
    try:
        ...
        connection.execute(
            ...
            (
                now,
                token,
                symbol or str(quote.get("name") or token)[:80],
                price,
                price,
                price,
                price,
...
def register_manual_paper_routes(app, *, paper_db: Path, cache_db: Path) -> None:
    @app.post("/api/manual-paper/order")
    def manual_paper_order(payload: dict[str, Any]) -> dict[str, Any]:
