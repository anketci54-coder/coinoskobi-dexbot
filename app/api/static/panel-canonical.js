(() => {
  ...
  const money=v=>{const x=n(v);return x===null?'—':`${x<0?'-':''}$${Math.abs(x).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})}`};
  const price=v=>{const x=n(v);return x===null?'—':x.toLocaleString('tr-TR',{maximumFractionDigits:x<1?10:4})};
  const compact=v=>{const x=n(v);return x===null?'—':new Intl.NumberFormat('en-US',{notation:'compact',maximumFractionDigits:2}).format(x)};
  const pct=v=>{const x=n(v);return x===null?'—':`${x>0?'+':''}${x.toFixed(2)}%`};
  const short=v=>{const s=String(v||'').trim();return !s?'—':s.length>18?`${s.slice(0,8)}…${s.slice(-6)}`:s};
  const text=(id,v)=>{const e=$(id);if(e)e.textContent=v};
  const paint=(id,v)=>{const e=$(id);if(!e)return;e.classList.remove('pos','neg','warn');const x=n(v);if(x>0)e.classList.add('pos');else if(x<0)e.classList.add('neg')};
  const getJSON=async(url,opts={})=>{const r=await fetch(url,{cache:'no-store',...opts});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.detail||`${url} ${r.status}`);return data};

  function openPositions(){return Array.isArray(state.dashboard?.positions)?state.dashboard.positions:[]}
  function positionFor(row){const pool=norm(row?.pool),t0=norm(row?.base_token||row?.token0),t1=norm(row?.token1);return openPositions().find(p=>String(p?.status||'OPEN').toUpperCase()!=='CLOSED'&&((pool&&norm(p.pool)===pool)||(norm(p.token)&&(norm(p.token)===t0||norm(p.token)===t1))))||null}
  function activeRows(source){return openPositions().map(p=>source.find(r=>positionFor(r)?.id===p.id)||{pool:p.pool,base_token:p.token,token0:p.token,display_name:p.symbol,state:'ACTIVE',price_usd:p.current_price??p.entry_price,seismic:{score:p?.entry_evidence?.score??null},_active:p}).filter(Boolean)}
  function radarRows(){const source=state.universe?.available&&Array.isArray(state.universe.rows)?state.universe.rows:[];if(state.filter==='ACTIVE')return activeRows(source);return source.filter(r=>n(r?.seismic?.score)!==null&&n(r.seismic.score)>0).filter(r=>state.filter==='ALL'||String(r.state||'').toUpperCase()===state.filter).sort((a,b)=>(n(b?.seismic?.score)||0)-(n(a?.seismic?.score)||0))}
  function rowName(r){return r?.display_name||r?.base_symbol||short(r?.base_token||r?.token0)||'POOL'}
  function rowState(r,p){return p||r?._active?'ACTIVE':String(r?.state||'COLD').toUpperCase()}
  function stateClass(v){const x=String(v||'').toLowerCase();return ['cold','warm','hot','active'].includes(x)?x:'cold'}

  function renderTop(){if(!state.dashboard){['balance','dailyPnl','dailyDate','totalPnl','totalPnlPct','openCount','openInvestment','riskUsed','riskAmount'].forEach(id=>text(id,'—'));paint('dailyPnl',null);paint('totalPnl',null);return}const s=state.dashboard.summary||{};text('balance',money(s.equity));text('dailyPnl',money(s.daily_pnl));paint('dailyPnl',s.daily_pnl);text('dailyDate',s.local_date||'—');text('totalPnl',money(s.total_pnl));paint('totalPnl',s.total_pnl);text('totalPnlPct',s.starting_capital?pct((n(s.total_pnl)||0)/(n(s.starting_capital)||1)*100):'—');text('openCount',String(s.open_count??0));text('openInvestment',money(s.open_investment));text('riskUsed',pct(s.risk_used_pct));text('riskAmount',money(s.open_risk));}

  async function refreshTickers(){try{const rows=await Promise.all(['BTCUSDT','ETHUSDT'].map(s=>getJSON(`https://api.binance.com/api/v3/ticker/24hr?symbol=${s}`)));[['btc',rows[0]],['eth',rows[1]]].forEach(([k,d])=>{text(`${k}Price`,price(d.lastPrice));text(`${k}Change`,pct(d.priceChangePercent));const e=$(`${k}Change`);if(e){e.classList.toggle('pos',n(d.priceChangePercent)>0);e.classList.toggle('neg',n(d.priceChangePercent)<0)}})}catch(e){text('btcChange','VERİ YOK');text('ethChange','VERİ YOK')}}
  ...
  function renderIntel(){if(window.__COINOSKOBI_REFINEMENT_V3_OWNS_INTEL)return;const body=$('walletRows');if(!state.dashboard){['walletCount','whaleCount','successCount'].forEach(id=>text(id,'—'));if(body)body.innerHTML='<tr><td colspan="3">VERİ YOK</td></tr>';return}const i=state.dashboard.intelligence||{};text('walletCount',String(i.tracked_wallets??0));text('whaleCount',String(i.active_whales??0));text('successCount',String(i.successful_wallets??0));const rows=walletRows().slice(0,5);if(body)body.innerHTML=rows.length?rows.map(r=>`<tr><td title="${esc(r.wallet_uid||r.wallet_id||'')}">${esc(short(r.wallet_uid||r.wallet_id))}</td><td>${esc(String(r.success_state||'UNKNOWN').toUpperCase())}</td><td>${esc(String(r.whale_state||'UNKNOWN').toUpperCase())}</td></tr>`).join(''):'<tr><td colspan="3">Detay kaydı yok</td></tr>'}

  function renderNews(){if(window.__COINOSKOBI_REFINEMENT_V3_OWNS_INTEL)return;const a=$('newsStream'),b=$('calendarStream');if(!state.dashboard){text('newsTitle','HABER AKIŞI');text('newsMeta','VERİ YOK');if(a)a.innerHTML='<div class="news-empty">Haber verisi alınamadı.</div>';if(b)b.innerHTML='<div class="news-empty">Takvim verisi alınamadı.</div>';return}const payload=state.dashboard.news||{},items=Array.isArray(payload.items)?payload.items:[],calendar=Array.isArray(payload.calendar)?payload.calendar:[];text('newsTitle',`HABER AKIŞI (${items.length})`);text('newsMeta',payload.source||'KAYNAK BAĞLI DEĞİL');if(a)a.innerHTML=items.length?items.map(x=>`<div class="news-item ${x.is_new?'neg':''}">${esc(x.title||x.text||'—')}</div>`).join(''):'<div class="news-empty">Gerçek haber kaynağı panel backend’ine bağlı değil. Sahte haber gösterilmiyor.</div>';if(b)b.innerHTML=calendar.length?calendar.map(x=>`<div class="news-item">${esc(x.title||x.text||'—')}</div>`).join(''):'<div class="news-empty">Gerçek ekonomik takvim kaynağı bağlı değil. Sahte etkinlik gösterilmiyor.</div>'}

  function renderOperations(){const e=$('systemState');if(!state.operations){text('systemState','VERİ YOK');text('providerState','VERİ YOK');if(e)e.className='warn';return}const o=state.operations,sys=o.system||{};text('systemState',sys.label||'—');const stateName=String(sys.state||'').toUpperCase();if(e)e.className=stateName==='HEALTHY'?'pos':stateName==='DEGRADED'?'warn':'';text('providerState',sys.data_label||'—');}

  function addBubble(textValue,kind='vezir'){const chat=$('chatBody');if(!chat)return;const d=document.createElement('div');d.className=`bubble ${kind==='user'?'user':''}`;d.textContent=textValue;chat.appendChild(d);chat.scrollTop=chat.scrollHeight}
  ...
  function clearLiveSnapshot(){state.snapshotGeneration+=1;state.dashboard=null;state.universe=null;state.watch=null;state.watchSummary=null;state.operations=null;state.selected=null;clearRestoredModals();renderTop();renderRadar();renderWatch();renderIntel();renderNews();renderOperations();text('updatedAt','—')}
  async function refresh(){if(document.hidden)return;const generation=state.snapshotGeneration;if(state.refreshing===generation)return;state.refreshing=generation;try{const [d,u,w,ws,o]=await Promise.allSettled([getJSON('/api/dashboard'),getJSON('/api/universe-panel'),getJSON('/api/watch-probes'),getJSON('/api/watch-probes-summary-v2'),getJSON('/api/operations-summary')]);if(generation!==state.snapshotGeneration)return;state.dashboard=d.status==='fulfilled'?d.value:null;if(!state.dashboard)closeTicket();state.universe=u.status==='fulfilled'?u.value:null;state.watch=w.status==='fulfilled'?w.value:null;state.watchSummary=ws.status==='fulfilled'?ws.value:null;state.operations=o.status==='fulfilled'?o.value:null;renderTop();renderRadar();renderWatch();renderIntel();renderNews();renderOperations();const successCount=[d,u,w,ws,o].filter(x=>x.status==='fulfilled').length,stamp=new Date().toLocaleTimeString('tr-TR');text('updatedAt',successCount===5?stamp:successCount>0?`${stamp} · KISMİ`:'VERİ YOK')}finally{if(state.refreshing===generation)state.refreshing=null}}
