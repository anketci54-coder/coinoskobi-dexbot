...
def _ratio_reward_to_risk(
    *, entry_price, take_profit_price, stop_loss_price
    ...
    if (
        entry is None or tp is None or sl is None
        or entry <= 0 or tp <= entry or sl >= entry or sl <= 0
    ):
        return None
    risk = entry - sl
...
def build_tactical_truth(
    *, market_context=None, execution_cost=None, paper=None,
    tp_multiplier=None, sl_multiplier=None,
):
    """Compose runtime truth without fixed TP/SL authority.

    Multiplier arguments are accepted only for compatibility and ignored.
    Dynamic trade-control state is the source of exit levels.
    """
