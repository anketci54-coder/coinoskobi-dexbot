...
cp -a "$HIST" "$HIST.pre_rdnt_boundary.$TS.bak"
...
sym="RDNT_TRY"
if sym not in by:
    raise SystemExit("MISSING_HISTORICAL_ROW:RDNT_TRY")

by[sym].update({
    "trading_start_local":"2024-04-03 11:00",
    "trading_start_status":"VERIFIED",
    "membership_from_semantics":"EXACT_LISTING_START",
    "listing_source_url":"https://www.binance.tr/tr/blog/Geli%C5%9Fmeler/783caa9e48e641f78f6a6166dafd7a8d"
})
...
print("RDNT_TRY 2024-04-03 11:00 EXACT_LISTING_START")
PY
...
print("RDNT_TRY")
for e in (cat.get("episode_windows") or {}).get("RDNT_TRY",[]):
    print(e)
PY
...
echo
echo "RDNT_STRICT_BOUNDARY=PASS"
