...
def test_static_safety_floor_cannot_be_weakened():
    r = evaluate_hybrid_exit(
        entry_price=1,
        current_price=0.89,
        highest_price=1,
        static_sl_price=0.90,
    )

    assert r.action == "EXIT"
    assert r.reason == "STATIC_SAFETY_FLOOR"
    assert r.exit_now is True
...
def test_profit_retrace_hits_dynamic_protection():
    r = evaluate_hybrid_exit(
        ...
        highest_price=1.50,
        static_sl_price=0.90,
        liquidity_health=0.50,
        trend_health=0.50,
        price_impact_health=0.50,
    )
