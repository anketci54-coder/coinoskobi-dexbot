...
def _event(tx, idx, block, subscription="sub-1", removed=False):
    return {
        "jsonrpc": "2.0",
...
def _ack(subscription="sub-1"):
    return {"jsonrpc": "2.0", "id": 1, "result": subscription}
...
class Factory:
    ...
    def __call__(self, url, **kwargs):
        ...
        return CM(self.sockets.popleft())


def test_wss_to_composition_observation_chain():
    composition = RuntimeIntelligenceComposition()
    accepted = []
    ...
    runtime = NativeWSSRuntime(
        ...
        connect_factory=Factory([
            FakeWS([
                _ack(),
                _event("0xaaa", "0x1", "0x10"),
                _event("0xbbb", "0x2", "0x10"),
            ])
        ]),
        on_event=on_event,
    )
    result = asyncio.run(runtime.run(max_events=2))
    ...
    assert context["runtime_connected"]["phase5_market"] is True
    assert context["runtime_connected"]["phase7_flow_regime"] is True
    assert context["runtime_connected"]["phase8_native_binding"] is True
...
def test_duplicate_and_removed_never_reach_callback():
    ...
    async def on_event(event):
        accepted.append(event["event_identity"])

    duplicate = _event("0xaaa", "0x1", "0x10")
    runtime = NativeWSSRuntime(
        "wss://example",
        "0xpair",
        connect_factory=Factory([
            FakeWS([
                ...
                duplicate,
                duplicate,
                _event("0xremoved", "0x2", "0x10", removed=True),
                _event("0xbbb", "0x3", "0x10"),
            ])
        ]),
        on_event=on_event,
    )
    result = asyncio.run(runtime.run(max_events=2))
    assert accepted == ["0xaaa:0x1", "0xbbb:0x3"]
...
def test_reconnect_preserves_seen_memory():
    ...
    second = FakeWS([
        _ack("sub-2"),
        _event("0xaaa", "0x1", "0x10", subscription="sub-2"),
        _event("0xbbb", "0x2", "0x10", subscription="sub-2"),
    ])
    ...
    runtime = NativeWSSRuntime(
        "wss://example",
        "0xpair",
        connect_factory=Factory([first, second]),
        sleep_func=no_sleep,
        max_reconnects=2,
    )
    result = asyncio.run(runtime.run(max_events=2))
...
def test_readmodels_cannot_upgrade_candidate():
    ...
    result = composition.build(
        "0xtoken",
        wallet_id="bsc:0xabc",
        adversary_key="actor:1",
    )
    assert result["can_upgrade_candidate"] is False
