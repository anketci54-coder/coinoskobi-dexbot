...
def test_gecko_source_binding():
    source = get_source("geckoterminal")
    assert source["enabled"] is True
    assert "bsc" in source["networks"]

    binding = get_source_network("geckoterminal", "bsc")
...
def test_disabled_network_cannot_route_source():
    with pytest.raises(RuntimeError):
        get_source_network("geckoterminal", "base")


def test_source_network_binding_is_explicit():
    with pytest.raises(RuntimeError):
        get_source_network("geckoterminal", "ethereum")
